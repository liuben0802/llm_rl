...
up:          ; docker compose up -d llamafactory verl
...
board:
	docker run --rm -p 6006:6006 \
		...
		tensorflow/tensorflow tensorboard --logdir /saves --host 0.0.0.0
shell-lf:    ; docker exec -it rec_llamafactory bash
shell-verl:  ; docker exec -it rec_verl bash
clean:       ; rm -rf workspace/logs/*; docker compose down --remove-orphans
