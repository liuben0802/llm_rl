...
生成三份数据集，输出到 /data（← 宿主机 /data4/datas）：
  /data/sft_train.jsonl        LLaMA-Factory sharegpt 格式
  /data/rm_train.jsonl         LLaMA-Factory pairwise 格式
...
TEACHER_BASE  = "http://host.docker.internal:8000/v1"
TEACHER_MODEL = "Qwen2.5-72B-Instruct"
OUT = Path("/data")     # ← 容器内 /data = 宿主机 /data4/datas
...
def _user_truncated(s: dict) -> str:
    """SFT / RM 用截断版 user prompt"""
...
def _user_full(s: dict) -> str:
    """GRPO / 推理用完整 user prompt（不截断）"""
    return build_user_prompt(
        shop_name=s["shop_name"], categories=s["categories"],
        brands=s["brands"], products_90d=s["products_90d"],
        products_lastyear=s["products_lastyear"],
...
def make_rm(samples: list[dict], min_gap: float = 0.15):
    ...
    with open(path, "w", encoding="utf-8") as f:
        for s in tqdm(samples, desc="RM"):
            ...
            record = {"chosen": _msgs(chosen), "rejected": _msgs(rejected)}
            f.write(json.dumps(record, ensure_ascii=False) + "\n")
            kept += 1
...
# ────────────────────────────────────────────────────────────────
# GRPO 数据（SYSTEM_INFER + 完整 user，与线上推理完全一致）
# ────────────────────────────────────────────────────────────────
def make_grpo(samples: list[dict]):
    rows = []
    for s in samples:
        user = _user_full(s)          # 不截断
        rows.append({
            "prompt": json.dumps([
